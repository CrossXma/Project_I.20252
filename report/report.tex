...
\title{\textbf{Static Heuristic Malware Detection Engine:\\PE Structure and Assembly Instruction Analysis}}
\author{
    \textbf{Project Group Members:} \\
    CrossXma (\href{mailto:congyb2006@gmail.com}{congyb2006@gmail.com}) \\
    Phuong Mai (meeGit-dpm) (\href{mailto:hangmai501h6@gmail.com}{hangmai501h6@gmail.com})
}
\date{June 22, 2026}
...
% ==============================================================================
\section{Member Contribution Workload}
% ==============================================================================
This project was carried out as a collaborative effort between two members: \textbf{CrossXma} and \textbf{Phuong Mai} (git author: \textit{meeGit-dpm}). The division of tasks was structured to run the data collection, reverse-engineering feature engineering, and the heuristic model validation in parallel. 

Table~\ref{tab:contributions} summarizes the specific contributions and estimated workload distribution.
...
\toprule
\textbf{Member} & \textbf{Core Contributions} & \textbf{Workload Share} \\
\midrule
\textbf{CrossXma} & 
...
\textbullet~Outlined multi-threaded MalwareBazaar downloader. & 40\% \\
\midrule
\textbf{Phuong Mai} & 
...
\textbullet~Conducted error analysis on False Positives and compiled results. & 60\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Comparison of Detection Metrics Before and After Optimizations}
\label{tab:metrics_comparison}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Initial Heuristic Model} & \textbf{Optimized Heuristic Model} & \textbf{Absolute Improvement} \\
\midrule
True Positives (TP) & 593 & 760 & +167 \\
False Negatives (FN) & 211 & 44 & -167 \\
True Negatives (TN) & 933 & 949 & +16 \\
False Positives (FP) & 60 & 44 & -16 \\
Accuracy & 84.92\% & 95.10\% & +10.18\% \\
Precision & 90.81\% & 94.53\% & +3.72\% \\
Recall (Sensitivity) & 73.76\% & 94.53\% & +20.77\% \\
F1-Score & 81.40\% & 94.53\% & +13.13\% \\
\bottomrule
\end{tabular}
\end{table}
...
The remaining False Positives are mainly administrative files and .NET resource packages:
\begin{itemize}
    \item \textbf{\texttt{DWWIN.EXE} (Doctor Watson Error Debugger)}: Evaluated at a Score of 55. Although its score dropped from 115 thanks to the SDK and DLL complexity discounts, it still exceeds the threshold because it contains multiple process hollowing/injection APIs needed to debug and report on crashed processes.
    \item \textbf{.NET Resource assemblies (e.g. \texttt{Microsoft.Dism.Powershell.Resources.dll})}: Classified as malware because they have few or zero imports, triggering the \texttt{Few imports} penalty, and contain localized strings matching suspicious command keywords.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Significant Detection Recall Boost:} By adding the Dropper heuristic and bypassing library discounts for high-entropy stubs, the model successfully recovered $167$ previously missed malware samples, raising the \textbf{Recall from 73.76\% to 94.53\%}.
...
To improve precision and adapt the engine to sophisticated evasion techniques, future research will target:
\begin{enumerate}
    \item \textbf{Authenticode Signature Validation}: Incorporating checks for trusted digital certificates from verified publishers (e.g., Microsoft) to automatically apply scoring discounts to trusted administrative files.
    \item \textbf{API Call Sequencing and Control Flow Graphs (CFG)}: Transitioning from basic IAT import tracking to parsing instruction-level calling paths, ensuring that flagged APIs are actually called in malicious contexts rather than simply declared in the imports.
    \item \textbf{Machine Learning Classification}: Applying machine learning classifiers (such as Random Forests or Support Vector Machines) to the extracted PE/ASM feature vectors to learn non-linear patterns, replacing manually-tuned heuristic weights.
\end{enumerate}

Overall, the developed static analysis framework provides a secure, lightweight, and highly sensitive first line of defense for processing binary samples within isolated lab environments.
\end{document}
