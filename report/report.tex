...
\begin{itemize}
    ...
    \item \texttt{.rdata}: Contains read-only variables, debug directories, and the Import Address Table (IAT).
...
Reads the target executable to extract structure features. It parses the NT header directories, checks the number of sections, calculates the Shannon entropy for each individual section, builds the Import Address Table (IAT) mapping, and exports lists. The output is written to a JSON feature registry.

\subsubsection{ASM Feature Extractor (\texttt{asm\_extractor.py})}
Disassembles the \texttt{.text} section bytes. It initializes a Capstone disassembly session (\texttt{Cs(CS\_ARCH\_X86, CS\_MODE\_32)}) and converts instructions into opcodes. It extracts instruction frequencies, lists the top 30 most common mnemonics, generates 3-gram instruction sequences, parses strings to identify suspicious commands, and checks constants for common networking ports.
...
Processes benign and malware JSON feature registries. It evaluates classification metrics, outputs performance statistics, and lists false classifications with details on why they occurred.
...
The performance of the heuristic engine was validated against a dataset of 296 files, consisting of:
\begin{itemize}
    \item \textbf{Benign Samples}: 993 legitimate Windows executables and DLLs (compiled from standard system folders like \texttt{C:\textbackslash{}Windows\textbackslash{}System32}).
    \item \textbf{Malware Samples}: 804 wild malware samples collected from MalwareBazaar.
\end{itemize}

\subsection{Detection Metrics}
The classification results are summarized in Table~\ref{tab:confusion} and the evaluated metrics are detailed in Table~\ref{tab:metrics}.

\begin{table}[h]
\centering
\caption{Malware Detector Confusion Matrix}
\label{tab:confusion}
\begin{tabular}{lcc}
\toprule
...
\begin{table}[h]
\centering
\caption{Performance Metrics Evaluation}
\label{tab:metrics}
\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Evaluated Value} \\
\midrule
Accuracy & 95.10\% \\
Precision & 94.53\% \\
...
\subsection{Some False Positives Case Studies}
...
The 94.53\% recall achieved by the engine indicates that the selected scoring weights are highly sensitive to malware patterns. Key indicators such as high section entropy ($>7.1$), write-execute violations ($W^X$), size mismatches, and the presence of decryption loops successfully identified obfuscated or packed malware. 

However, the evaluation highlights a limitation of static heuristic engines: the difficulty of distinguishing malicious behavior from legitimate administrative tool capabilities. Legitimate system binaries designed for debugging (\texttt{dbgeng.dll}), system command interpreters (\texttt{cmd.exe}), or licensing protection systems often mirror packer and process injection signatures.
